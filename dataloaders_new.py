...
class Datasets(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mix, _ = sf.read(
            self.mix_audio[index], start=rand_start, stop=stop, dtype="float32"
        )
        for i in range(len(ref)):
            ref[i], _ = sf.read(
                self.ref_audio[i][index], start=rand_start, stop=stop, dtype="float32"
            )
        return {"mix": torch.from_numpy(mix), "ref": [torch.from_numpy(r) for r in ref]}
